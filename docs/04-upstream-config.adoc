...
Some upstream providers enforce hard request-rate limits (e.g. 30 req/s on public endpoints).
The `rate-limit` option lets you cap the number of requests per second Dshackle sends to an individual upstream.
When an upstream reaches its limit it is skipped immediately and the next available upstream is used instead — no queuing, no errors to the caller.
    ...
    connection:
      generic:
        rpc:
          ...
          rate-limit: 20    # max 20 req/s; excess requests are routed to the next upstream
----

* `rate-limit` is optional. Omitting it (or setting it to `0` / negative) means unlimited.
* The limit applies per upstream instance using a sliding 1-second window.
