...
class JsonRpcHttpReaderSpec extends Specification {
    ...
    def "Make a request"() {
        setup:
        JsonRpcHttpReader client = new JsonRpcHttpReader("localhost:${mockServer.port}", 50, 50, metrics, Schedulers.boundedElastic(),null, null, [:], Duration.ofSeconds(60))
        def resp = '{' +
                '  "jsonrpc": "2.0",' +
                '  "result": "0x98de45",' +
                '  "error": null,' +
    ...
    def "Make a request with bearer auth"() {
        ...
        JsonRpcHttpReader client = new JsonRpcHttpReader("localhost:${mockServer.port}", 50, 50, metrics, Schedulers.boundedElastic(), null, null, [:], Duration.ofSeconds(60), bearerAuth)
        def resp = '{' +
                '  "jsonrpc": "2.0",' +
                '  "result": "0x98de45",' +
                '  "error": null,' +
    ...
    def "Produces RPC Exception on error status code"() {
        setup:
        def client = new JsonRpcHttpReader("localhost:${mockServer.port}", 50, 50, metrics, Schedulers.boundedElastic(), null, null, [:], Duration.ofSeconds(60))
        mockServer.when(
                HttpRequest.request()
        ).respond(
