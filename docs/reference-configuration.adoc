...
accessLog:
  ...
  chains:
    - chain: ethereum
      enabled: false
      filename: /var/log/dshackle/ethereum.jsonl
      include-messages: true
      filesize: 50mb
    - chain: bitcoin
      enabled: false
      filename: /var/log/dshackle/bitcoin.jsonl
----
...
| `enabled`
| `false`
| Master switch for the access logging system. Required for any logging to occur.
...
| `errors-only`
| `false`
| When enabled, only failed `NativeCall` events (i.e., `succeed == false`) are logged.
All other event types and successful calls are dropped.
This is useful for debugging RPC errors without the noise of successful requests.

| `filename`
| _(optional)_
| Path to the global access log file. Omit to skip global logging (useful when only per-chain or per-upstream logs are needed).

| `filesize`
| _(none)_
| Maximum file size before rotation (e.g., `100mb`, `1gb`, `500kb`). When a log file exceeds this size, it is renamed with a timestamp suffix and a new file is started. Existing logs are also rotated on startup.

| `chains`
|
| List of per-chain log targets. Each entry routes events for a specific blockchain to a separate file.

| `chains[].chain`
|
| Blockchain id (e.g., `ethereum`, `bitcoin`, `optimism`)

| `chains[].enabled`
| `true`
| Enable/disable this chain target. Allows pre-configuring targets that can be toggled on for debugging.

| `chains[].filename`
|
| Path to the chain-specific log file

| `chains[].include-messages`
| _(inherits global)_
| Override global `include-messages` for this chain target

| `chains[].errors-only`
| _(inherits global)_
| Override global `errors-only` for this chain target

| `chains[].filesize`
| _(inherits global)_
| Override global `filesize` for this chain target

|===

==== Per-Upstream Access Log

Each upstream can also have its own access log, configured alongside other upstream settings like `options`.
Only `NativeCall` events with a matching `upstreamId` are routed to upstream-specific logs.
...
cluster:
  upstreams:
    - id: infura-opt
      ...
      access-log:
        enabled: false
        filename: /var/log/dshackle/infura_opt.jsonl
        include-messages: true
      connection:
        generic:
          rpc:
...
| `access-log.enabled`
| `true`
| Enable/disable this upstream log target

| `access-log.filename`
|
| Path to the upstream-specific log file (required)

| `access-log.include-messages`
| `false`
| Include request/response payloads

| `access-log.errors-only`
| `false`
| Log only failed NativeCall events

| `access-log.filesize`
| _(none)_
| Maximum file size before rotation (e.g., `100mb`, `1gb`)
...
| `access-log`
| no
| Per-upstream access log configuration. See <<accessLog>>
...
| `additional-settings`
| no
| Additional upstream settings. Currently supports `manual-lower-bounds`. See xref:04-upstream-config.adoc#_manual_lower_bounds[Manual Lower Bounds]

| `additional-settings.manual-lower-bounds`
| no
| Map of lower bound overrides by type (`block`, `logs`, `receipts`, `tx`, `state`).
Each entry has `type` (`FIXED` or `HEAD`) and `value` (block number or negative offset from head).
Prevents routing requests for historical data to nodes that don't have it.
...
| `rpc.rate-limit`
a| Maximum number of requests per second to send to this upstream, regardless of its sync state.
When the limit is reached, the upstream is skipped in favour of the next available upstream rather than queuing or returning an error.
Useful for public endpoints that enforce hard rate limits (e.g. 30 req/s). +
Optional. Default is unlimited (`null`). Values ≤ 0 are treated as unlimited. +
Example: `rate-limit: 200`

| `rpc.sync-rate-limit`
a| Maximum requests per second when the upstream status is *SYNCING* (node is far behind the chain head).
Limits traffic to a catching-up node so it can sync without being overwhelmed.
Only evaluated when the upstream is in SYNCING state; ignored otherwise. +
Optional. Default is unlimited. +
Example: `sync-rate-limit: 2`

| `rpc.lagging-rate-limit`
a| Maximum requests per second when the upstream status is *LAGGING* (node is slightly behind the chain head).
Provides a lenient cap so a mildly behind node can recover without being flooded.
Only evaluated when the upstream is in LAGGING state; ignored otherwise. +
Optional. Default is unlimited. +
Example: `lagging-rate-limit: 20`
