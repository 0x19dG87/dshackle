...
Dshackle supports three independent, optional rate-limit fields on the `rpc` block.
All three use a sliding 1-second token-bucket window and skip the upstream (routing to the next available one) when the budget is exhausted.
...
| `rate-limit`
| Always — caps total throughput to this upstream regardless of sync state.

| `sync-rate-limit`
| Only when the upstream status is *SYNCING* (node is far behind the chain head, e.g. 10k+ blocks). Keeps a syncing node from being overwhelmed while it catches up.

| `lagging-rate-limit`
| Only when the upstream status is *LAGGING* (node is slightly behind the chain head). Provides a lenient cap so the node can recover without being flooded.
|===

All three fields are independently optional; any combination is valid.
    ...
    connection:
      ethereum:
        rpc:
          ...
          rate-limit: 200          # always-on cap: max 200 req/s
          sync-rate-limit: 2       # when SYNCING: only 2 req/s
          lagging-rate-limit: 20   # when LAGGING: only 20 req/s
----

* All fields are optional. Omitting a field (or setting it to `0` / negative) means unlimited for that state.
* If _all_ upstreams in a chain are simultaneously rate-limited, the caller receives the normal "no upstream available" response.
* `sync-rate-limit` and `lagging-rate-limit` are mutually exclusive at runtime — only the one matching the current upstream status is evaluated.
...
Some blockchains expose the same functionality under different method names.
For example, Arbitrum uses `arbtrace_filter` instead of the standard `trace_filter`.
If a client calls a method that the upstream node does not support under that name, the request will fail.

The `alias` field lets you configure a transparent method rename: the client calls the original name, Dshackle accepts it and forwards the request to the upstream using the aliased name.

NOTE: The alias only affects the method name sent to the upstream. All routing, availability checks, and quorum logic continue to use the original method name as configured in `enabled`.
...
upstreams:
  - id: arbitrum-node
    ...
    methods:
      enabled:
        - name: trace_filter
          alias: arbtrace_filter
        - name: trace_call
          alias: arbtrace_call
        - name: trace_block
          alias: arbtrace_block
        - name: trace_get
          alias: arbtrace_get
        - name: trace_transaction
          alias: arbtrace_transaction
        - name: trace_replayTransaction
          alias: arbtrace_replayTransaction
        - name: trace_replayBlockTransactions
          alias: arbtrace_replayBlockTransactions
        - name: trace_callMany
          alias: arbtrace_callMany
    connection:
      generic:
        rpc:
...
With this configuration any client calling `trace_filter` will have its request forwarded to the Arbitrum node as `arbtrace_filter`.
All other methods remain unchanged and fully available.
...
Dshackle automatically detects the earliest block available on each upstream (archive depth) by probing the node at startup.
However, you can override or pre-set these bounds using `additional-settings.manual-lower-bounds` to avoid routing requests for historical data to nodes that don't have it.

This is especially important for nodes that only cover part of a chain's history — for example, a post-Bedrock Optimism node that does not store pre-Bedrock blocks (before block 105,235,063).
Without a manual lower bound, Dshackle may still try such a node for old blocks, which causes errors, populates internal failure caches, and can temporarily prevent retries to other upstreams.

Each bound type corresponds to a different kind of historical data:

[cols="1,4"]
|===
| Type | Data covered

| `block` | Block headers and body (`eth_getBlockByHash`, `eth_getBlockByNumber`)
| `logs` | Event log index (`eth_getLogs`)
| `receipts` | Transaction receipts (`eth_getTransactionReceipt`, `eth_getBlockReceipts`)
| `tx` | Transaction index (`eth_getTransactionByHash`)
| `state` | Historical state for calls (`eth_call`, `eth_getStorageAt` at a past block)
|===
...
| `FIXED` | A fixed block number. The upstream will not be used for requests below this block.
| `HEAD` | A negative offset from the current chain head. E.g. `-1000` means "only serve the last 1000 blocks".
|===

.Example: post-Bedrock Optimism node without pre-Bedrock history
[source,yaml]
----
upstreams:
  - id: optimism-reth
    ...
    additional-settings:
      manual-lower-bounds:
        block:
          type: FIXED
          value: 105235063   # Optimism Bedrock upgrade block
        logs:
          type: FIXED
          value: 105235063
        receipts:
...
.Example: non-archive node serving only recent blocks (last 128)
[source,yaml]
----
upstreams:
  - id: pruned-node
...
NOTE: When no manual bound is set, Dshackle auto-detects the lower bound by binary-searching the node's available history.
The manual bound takes effect immediately at startup, before auto-detection runs, which prevents routing errors during the initial period.
