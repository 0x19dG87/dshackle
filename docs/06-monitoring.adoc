...
To log only failed requests (useful for debugging RPC errors), add `errors-only: true`:
...
When `errors-only` is enabled, only failed `NativeCall` events are logged; all successful calls and other event types are dropped.

=== Per-Chain and Per-Upstream Logging

For debugging specific chains or upstreams, you can route access log events to separate files.
This is useful when you have heavy traffic across many chains and need to isolate events for a specific one.

Per-chain logging is configured in the `accessLog` section:
...
accessLog:
  ...
  chains:
    - chain: optimism
      enabled: false
      filename: /var/log/dshackle/optimism.jsonl
      include-messages: true
    - chain: ethereum
      enabled: false
...
Per-upstream logging is configured on each upstream definition:
...
Each target has an `enabled` flag so configurations can stay in place and be toggled on/off as needed for debugging.
The global `filename` is optional -- omit it to avoid writing a global log when only per-chain or per-upstream logs are needed.
`include-messages` and `errors-only` on targets default to the global values when not specified.

NOTE: The `accessLog.enabled: true` master switch must be set for any logging to occur.
Per-upstream logs only match `NativeCall` events where the response includes the upstream's ID.
